...
int main(int argc, char* argv[])
{
	...
	if(argc != 4)
	{
		fprintf(stderr,"Usage: %s <num_locs - e.g. 17> <init_num_write - e.g. 5000> <reps - eg. 10.",argv[0]);
		exit(0);
	}
	...
	pmem_exp_init("/mnt/ramdisk/pmem_overwr_pool",REGION_SIZE,true);

    printf("offset,remaining_writes\n");
	for(int i=0;i<num_locs;i++)
	{
        ...
        {
            for (int j=0;j<init_num_write;j++)
            {
