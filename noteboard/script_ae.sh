...
postprocess_ths=${postprocess_ths:-3}
...
single_run() {
    ...
    for ((i = 0 ; i < exp_cnt ; i = i+1 )); do
        ...
        receive_noteboard $dimm_mnt/noteboard_$i tmp/$i
    done

    ./postprocess.py 'tmp/*' $postprocess_clm $postprocess_ths  | tee >(tail -n 1 > $final_res)
    rm -rf tmp
}
