...
export kv_port=${kv_port:-1111}
export kv_backing_file=${kv_backing_file:-/mnt/pmem0/wearlevel-covert}
...
start_remote_server() {
    ../common/remote_commands/start_kv_server.sh
}
stop_remote_server() {
...
single_run() {
    ...
    for ((i=0;i<rep_count;i++)) ; do
        ...
        taskset -c 10 ./receiver $remote_ip $kv_port > recvr_temp_log_$i &
        echo "Running sender"
        time_start=$(date +%s.%N)
        taskset -c 16-23 ./sender $remote_ip $kv_port > sender_temp_log_$i
        time_end=$(date +%s.%N)
